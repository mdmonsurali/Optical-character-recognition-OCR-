...
st.markdown('<p style="margin-top: -20px;">Extract Markdown-structured text from images, PDFs, or DOCX files using the Dolphin model.</p>', unsafe_allow_html=True)
st.markdown("---")
            ...
            if file_ext in ["png", "jpg", "jpeg"]:
                ...
                try:
                    ...
                    with tempfile.TemporaryDirectory() as tmp_dir:
                        ...
                        docx_buffer = io.BytesIO()
                        json_to_docx(all_json, output_path=docx_buffer, base_path=tmp_dir)
                        docx_buffer.seek(0)
                        st.session_state["ocr_docx_bytes"] = docx_buffer

                except Exception as e:
                    st.error(f"DOCX to image OCR error: {e}")
